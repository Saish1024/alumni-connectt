...
import { GraduationCap, Eye, EyeOff, ArrowLeft, Shield, AlertCircle } from 'lucide-react';
...
export default function LoginPage() {
    ...
    const [role, setRole] = useState<'student' | 'alumni' | 'faculty' | 'admin'>('student');
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Redirect on success — login re-throws on error (AuthContext does not if it only sets state)
            router.push('/dashboard');
        } catch {
            // Error is displayed by the error banner already
        }
    };
                ...
                <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl p-8 shadow-2xl">
                            ...
                            <form onSubmit={handleForgot} className="space-y-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-white/10 border border-white/20 text-white placeholder-white/40 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-400/50 transition-all font-[400]"
                                    />
                                </div>
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                    {(['student', 'alumni', 'faculty'] as const).map((r) => (
                                        <button
                                            ...
                                            className={`py-2 px-2 rounded-xl text-xs font-[600] transition-all border capitalize ${role === r
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    <input
                                        ...
                                        placeholder="••••••••"
                                        className="w-full bg-white/10 border border-white/20 text-white placeholder-white/40 rounded-xl px-4 py-3 pr-12 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-400/50 transition-all font-[400]"
                                    />
                    ...
                    {!forgotMode && (
                        <p className="text-center text-white/50 text-sm mt-6">
                            ...
                            <button onClick={() => router.push('/signup')} className="text-indigo-400 hover:text-indigo-300 font-[600] transition-colors">
                                Sign up for free
                            </button>
                        </p>
                    )}
