...
import { GraduationCap, ArrowLeft, ArrowRight, Eye, EyeOff, Check, Users, BookOpen, Briefcase, Shield } from 'lucide-react';
...
const roles = [
  ...
  },
  {
    key: 'admin',
    label: 'Administrator',
    desc: 'Manage platform users and content',
    icon: Shield,
...
export default function Signup() {
  ...
  const [form, setForm] = useState({
    ...
    institution: '', department: '', year: '', linkedin: '',
    agree: false,
  });
  ...
  if (step === 3) {
    return (
      <div className={`min-h-screen ${isDark ? 'dark' : ''} bg-gradient-to-br from-slate-950 via-indigo-950 to-purple-950 flex items-center justify-center p-4`}>
        <div className="w-full max-w-md">
          <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl p-10 shadow-2xl text-center">
            <div className="w-20 h-20 rounded-full bg-gradient-to-br from-green-400 to-emerald-600 flex items-center justify-center mx-auto mb-6 shadow-2xl shadow-green-500/40">
              <Check className="w-10 h-10 text-white" />
            </div>
            <h2 className="text-3xl font-[800] text-white mb-3">You're in! 🎉</h2>
            <p className="text-white/60 mb-8">Welcome to Alumni Connect! Your account has been created successfully.</p>
            <div className="space-y-3">
              <button
                onClick={() => navigate(roleRoutes[selectedRole])}
              ...
              <button onClick={() => navigate('/')} className="w-full py-3 text-white/50 hover:text-white text-sm transition-colors">
                Back to Home
              </button>
            </div>
          </div>
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'dark' : ''} bg-gradient-to-br from-slate-950 via-indigo-950 to-purple-950 flex items-center justify-center p-4 relative overflow-hidden`}>
      ...
      <div className="w-full max-w-lg relative">
        <button onClick={() => navigate('/')} className="flex items-center gap-2 text-white/70 hover:text-white mb-8 transition-colors text-sm font-[500]">
        ...
        <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl p-8 shadow-2xl">
          ...
          {step === 2 && (
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="John"
                    className="w-full bg-white/10 border border-white/20 text-white placeholder-white/40 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all"
                  />
                </div>
                <div>
              ...
              <div>
                <label className="block text-sm font-[600] text-white/80 mb-2">Institution / University</label>
                <input
                  required value={form.institution} onChange={e => setForm({ ...form, institution: e.target.value })}
                  placeholder="IIT Delhi, NIT Bombay..."
                  className="w-full bg-white/10 border border-white/20 text-white placeholder-white/40 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-[600] text-white/80 mb-2">Department</label>
                  <input
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-white/10 border border-white/20 text-white rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all">
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder="••••••••"
                    className="w-full bg-white/10 border border-white/20 text-white placeholder-white/40 rounded-xl px-4 py-3 pr-12 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all"
                  />
          ...
          <p className="text-center text-white/50 text-sm mt-6">
            ...
            <button onClick={() => navigate('/login')} className="text-indigo-400 hover:text-indigo-300 font-[600] transition-colors">
              Sign in
            </button>
          </p>
        </div>
