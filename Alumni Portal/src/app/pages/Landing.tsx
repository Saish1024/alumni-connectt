...
const alumni = [
  ...
  },
  {
    ...
    quote: 'The platform\'s quiz system and hackathons helped me build skills that led to my startup.',
  ...
  },
  {
    ...
    quote: 'The AI mentor matching found me the perfect mentor who guided me through my entire job search.',
        ...
        {menuOpen && (
          <div className="md:hidden bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-700 px-4 py-4 space-y-3">
            {['Features', 'How it Works', 'Success Stories'].map(item => (
              <a key={item} href="#" className="block text-sm text-slate-600 dark:text-slate-300 py-2">{item}</a>
            ))}
            <div className="flex gap-3 pt-2">
              <button onClick={() => navigate('/login')} className="flex-1 py-2.5 text-sm border border-slate-300 dark:border-slate-600 rounded-xl text-center font-[500]">Sign In</button>
              <button onClick={() => navigate('/signup')} className="flex-1 py-2.5 text-sm bg-gradient-to-r from-indigo-500 to-purple-600 text-white rounded-xl font-[600]">Sign Up</button>
            </div>
          </div>
        )}
      </nav>
